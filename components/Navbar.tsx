    ...
    <div className="relative inline-block text-left z-[100]">
      ...
      {isOpen && (
        <>
          ...
          <div className="absolute top-full right-0 mt-3 bg-white dark:bg-gray-800 rounded-2xl shadow-2xl border border-gray-100 dark:border-gray-700 py-2 w-48 animate-fade-in z-[120] overflow-hidden">
    ...
    <nav className={`fixed top-0 w-full z-50 transition-all duration-300 ${scrolled ? 'bg-white/95 dark:bg-gray-900/95 backdrop-blur-md border-b border-gray-200 dark:border-gray-800 py-2 shadow-lg' : 'bg-transparent py-6'}`}>
      <div className="max-w-7xl mx-auto px-6 flex justify-between items-center">
        ...
        <div className="flex items-center gap-3 cursor-pointer" onClick={() => onNavigate(AppView.LANDING)}>
          ...
          <span className={`text-2xl font-black tracking-tight ${scrolled ? 'text-gray-900 dark:text-white' : 'text-white'}`}>Caramelo</span>
        </div>

        {/* Pill Centralizada (Ajustada para o seu print) */}
        <div className={`hidden lg:flex items-center gap-6 px-8 py-2.5 rounded-full border transition-all ${scrolled ? 'bg-gray-50 border-gray-200 dark:bg-gray-800 dark:border-gray-700' : 'bg-white/10 border-white/20'}`}>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="hidden md:flex items-center gap-4 ml-2">
            <button onClick={() => onNavigate(AppView.LOGIN)} className={`text-sm font-black ${scrolled ? 'text-gray-700 dark:text-gray-200' : 'text-white'}`}>{t.login}</button>
      ...
      {/* Mobile Menu */}
      {mobileMenuOpen && (
        <div className="lg:hidden fixed inset-0 z-[200] bg-white dark:bg-gray-900 p-8 flex flex-col">
          ...
          <div className="flex flex-col gap-8 flex-1">
             <button onClick={() => { onNavigate(AppView.OUR_APPROACH); setMobileMenuOpen(false); }} className="text-left text-2xl font-black dark:text-white">{t.methodology}</button>
             <button onClick={() => { onNavigate(AppView.FOR_BUSINESS); setMobileMenuOpen(false); }} className="text-left text-2xl font-black dark:text-white">{t.forBusiness}</button>
             <button onClick={() => { onNavigate(AppView.PROFESSIONAL_HELP); setMobileMenuOpen(false); }} className="text-left text-2xl font-black dark:text-white">{t.help}</button>
             <div className="h-px bg-gray-100 dark:bg-gray-800"></div>
             <div className="space-y-4">
                <p className="text-xs font-black text-gray-400 uppercase tracking-widest">Idioma / Language</p>
                <div className="flex gap-4">
                   {['pt', 'en', 'es'].map(l => (
                      <button key={l} onClick={() => { onLanguageChange(l as Language); setMobileMenuOpen(false); }} className={`text-3xl p-3 rounded-2xl border ${language === l ? 'border-caramel-500 bg-caramel-50' : 'border-gray-100'}`}>
                         {l === 'pt' ? '🇧🇷' : l === 'en' ? '🇺🇸' : '🇪🇸'}
                      </button>
                   ))}
                </div>
